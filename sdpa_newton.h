...
class Newton {
  public:
    ...
    int SOCP_nBlock;
    int *SOCP_number;
    int **SOCP_constraint1;
    int **SOCP_constraint2;
    int **SOCP_blockIndex1;
    int **SOCP_blockIndex2;
    ...
    Newton();
    Newton(int m, int SDP_nBlock, int *SDP_blockStruct, int SOCP_nBlock, int *SOCP_blockStruct, int LP_nBlock);
    ~Newton();
    ...
    // 2008/03/12 kazuhide nakata
    void initialize_bMat(int m, Chordal &chordal, InputData &inputData, FILE *fpOut);
    ...
    void computeFormula_SDP(InputData &inputData, dd_real DenseRatio, dd_real Kappa);

    enum WHICH_DIRECTION { PREDICTOR, CORRECTOR };
    void compute_rMat(WHICH_DIRECTION direction, AverageComplementarity &mu, DirectionParameter &beta, Solutions &cuurentPt, WorkVariables &work);

    void Make_gVec(Newton::WHICH_DIRECTION direction, InputData &inputData, Solutions &currentPt, Residuals &currentRes, AverageComplementarity &mu, DirectionParameter &beta, Phase &phase, WorkVariables &work, ComputeTime &com);

    void calF1(dd_real &ret, DenseMatrix &G, SparseMatrix &Aj);
    void calF2(dd_real &ret, DenseMatrix &F, DenseMatrix &G, DenseMatrix &X, SparseMatrix &Aj, bool &hasF2Gcal);
    void calF3(dd_real &ret, DenseMatrix &F, DenseMatrix &G, DenseMatrix &X, DenseMatrix &invZ, SparseMatrix &Ai, SparseMatrix &Aj);

    // B_{i,j} = (X A_i Z^{-1}) \bullet A_j
    void compute_bMat_dense_SDP(InputData &inputData, Solutions &currentPt, WorkVariables &work, ComputeTime &com);
    ...
    void compute_bMat_sparse_SOCP(InputData &inputData, Solutions &currentPt, WorkVariables &work, ComputeTime &com);

    void compute_bMat_dense_LP(InputData &inputData, Solutions &currentPt, WorkVariables &work, ComputeTime &com);

    void compute_bMat_sparse_LP(InputData &inputData, Solutions &currentPt, WorkVariables &work, ComputeTime &com);

    void Make_bMat(InputData &inputData, Solutions &currentPt, WorkVariables &work, ComputeTime &com);
    ...
    bool compute_DyVec(Newton::WHICH_DIRECTION direction, InputData &inputData, Solutions &currentPt, WorkVariables &work, ComputeTime &com);

    void compute_DzMat(InputData &inputData, Residuals &currentRes, Phase &phase, ComputeTime &com);

    void compute_DxMat(Solutions &currentPt, WorkVariables &work, ComputeTime &com);

    bool Mehrotra(WHICH_DIRECTION direction, InputData &inputData, Solutions &currentPt, Residuals &currentRes, AverageComplementarity &mu, DirectionParameter &beta, Switch &reduction, Phase &phase, WorkVariables &work, ComputeTime &com);
