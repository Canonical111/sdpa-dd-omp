...
% --- source binding -----------------------------------------------------------------------------
% doc/build.sh writes srchash.tex holding the SHA-256 of THIS file, and the hash is then stamped
% into the PDF's Info dictionary via \pdfinfo, which pdftex writes as raw ASCII. That is what lets
% tests/check_doc_pdf.sh verify, with nothing but grep and sha256sum, that the committed PDF was
% produced from the committed source. srchash.tex is generated and not tracked, so the file must
% still compile without it -- a bare `latexmk technical.tex` in a fresh clone builds an unstamped
% PDF rather than failing.
\IfFileExists{srchash.tex}{\input{srchash}}{\newcommand{\SourceHash}{unstamped}}
\ifdefined\pdfinfo
  \begingroup\edef\x{\endgroup\noexpand\pdfinfo{ /SourceSHA256 (\SourceHash) }}\x
\fi
\hypersetup{%
  pdftitle={sdpa-dd-omp: threading a double-double SDP solver},
  pdfauthor={sdpa-dd-omp},
  pdfsubject={Technical record: threading, reproducibility and the factorisation choice}}
...
Built from \code{doc/technical.tex}, SHA-256 \code{\SourceHash}.
\end{center}
...
The backward substitution is a \emph{dot product}. Row $i$'s off-diagonals all accumulate into the
same $x_i$, so splitting the row across workers reassociates a finite-precision sum --- and no
arrangement avoids it, because row $i-1$ genuinely reads $x_i$, so the forward pass's
defer-the-write trick does not transfer.

The question is therefore not \emph{whether} the answer changes but \emph{which} answer it changes
to, and it is worth being exact about what the sibling fork does, because an earlier draft of this
document got it wrong. \code{sdpa-gmp-omp} splits each row into a \textbf{fixed number of chunks}
($64$ by default, \emph{independent of the team size}), accumulates each chunk in a scratch at
twice the vector's precision, and folds the partials in a fixed order. That buys two properties:
the result is identical at $1$, $8$, $32$ and $64$ threads and across reruns, and each chunk rounds
once where the legacy loop rounded at every one of the row's $\sim$$800$ subtractions.

It does \textbf{not} reproduce the legacy left fold bit for bit, and the implementation does not
claim it does --- its own source says the parallel pass ``does change the answer'', which is why
it ships opt-in behind \code{SDPA\_SOLVE\_BACKWARD} and defaults to serial. Nor is the accuracy
claim a theorem: higher-precision pairwise-style accumulation being no worse than a left fold is a
design intent to be checked empirically, for every input, rather than proved.

\code{dd} is not blocked here by a missing widened accumulator. \code{qd\_real} --- four doubles,
$\sim$$64$ digits, declared in the same headers as \code{dd\_real} --- is a direct analogue of
GMP's doubled chunk scratch. What it would buy is the same thing GMP gets: a \emph{new
deterministic algorithm with a new answer}, not a way to parallelise the legacy reduction
unchanged. There is no finite-width argument by which ``enough extra digits'' proves equality with
a computation that rounded after every subtraction; the only general way to reproduce that left
fold exactly is to preserve its dependence order, which is precisely what removes the reduction.

So the choice on \code{dd} is ``accept and validate a new deterministic algorithm, probably
opt-in'' versus ``keep the current answer and keep the reduction serial''. This fork currently
takes the second, and the reason is the contract stated in \S8.1: results here are bit-identical to
the serial computation, and changing the backward pass would spend that for a fraction of one
phase.
...
numerical failure with nothing valid to print --- no iteration completed, or the updated $X$/$Z$
  ...
  \code{solveStatus = FAILURE}, no solution section \\
