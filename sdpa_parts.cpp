...
Parameter::Parameter(FILE *parameterFile) { readFile(parameterFile); }
...
StepLength::~StepLength() { terminate(); }

void StepLength::initialize(dd_real alphaP, dd_real alphaD) {
    primal = alphaP;
    dual = alphaD;
}
...
dd_real StepLength::minBlockVector(BlockVector &aVec) {
    int nBlock = aVec.nBlock;
    dd_real ret = aVec.ele[0].ele[0];
    dd_real tmp;
...
void StepLength::computeStepLength(Solutions &currentPt, Newton &newton, WorkVariables &work, ComputeTime &com) {
    dd_real alphaBD = 100.0;
...
#else
    minxInvDxEigenValue = Jal::getMinEigen(currentPt.invCholeskyX, newton.DxMat, work);
// rMessage("minxInvDxEigenValue = " << minxInvDxEigenValue);
#endif
...
#else
    minzInvDzEigenValue = Jal::getMinEigen(currentPt.invCholeskyZ, newton.DzMat, work);
// rMessage("minzInvDzEigenValue = " << minzInvDzEigenValue);
#endif
...
void StepLength::MehrotraPredictor(InputData &inputData, Solutions &currentPt, Phase &phase, Newton &newton, WorkVariables &work, ComputeTime &com) {
#if 1
    primal = dual = 0.9;
#else
    ...
    if (phase.value == SolveInfo::noINFO || phase.value == SolveInfo::dFEAS) {
    ...
    if (phase.value == SolveInfo::noINFO || phase.value == SolveInfo::pFEAS) {
        // dual is infeasible
        if (dual > 1.0) {
            dual = 1.0;
        }
...
void StepLength::MehrotraCorrector(InputData &inputData, Solutions &currentPt, Phase &phase, Switch &reduction, Newton &newton, AverageComplementarity &mu, RatioInitResCurrentRes &theta, WorkVariables &work, Parameter &param, ComputeTime &com) {
    ...
    if (reduction.switchType == Switch::ON && (phase.value == SolveInfo::noINFO || phase.value == SolveInfo::pFEAS || phase.value == SolveInfo::dFEAS)) {
        dd_real xMatvMat;
        ...
        dd_real thetaMax = max((1.0 - primal) * theta.primal, (1.0 - dual) * theta.dual);
        dd_real muNew = mu.current + (primal * uMatzMat + dual * xMatvMat + primal * dual * uMatvMat) / nDim;
        dd_real alphaMax;
        ...
        while (thetaMax * thetaMax * mu.initial > xi * muNew) {
            ...
            thetaMax = max((1.0 - primal) * theta.primal, (1.0 - dual) * theta.dual);
            muNew = mu.current + (primal * uMatzMat + dual * xMatvMat + primal * dual * uMatvMat) / nDim;
...
//-------------------------------------------------
DirectionParameter::DirectionParameter(dd_real betaStar) { initialize(betaStar); }

DirectionParameter::~DirectionParameter() {
    // Nothing needs.
}

void DirectionParameter::initialize(dd_real betaStar) { value = betaStar; }

void DirectionParameter::MehrotraPredictor(Phase &phase, Switch &reduction, Parameter &param) {
    const dd_real nu = 2.0;
    if (phase.value == SolveInfo::pdFEAS) {
        value = 0.0;
    } else {
        value = param.betaBar;
...
void DirectionParameter::MehrotraCorrector(Phase &phase, StepLength &alpha, Solutions &currentPt, Newton &newton, AverageComplementarity &mu, Parameter &param) {
    int nDim = currentPt.nDim;
    ...
    dd_real muTarget = mu.current + (alpha.primal * uMatzMat + alpha.dual * xMatvMat + alpha.primal * alpha.dual * uMatvMat) / nDim;
...
Switch::Switch(SwitchType switchType) { initialize(switchType); }

Switch::~Switch() {
    // Nothing needs.
}

void Switch::initialize(SwitchType switchType) { this->switchType = switchType; }

void Switch::MehrotraPredictor(Phase &phase) {
    if (phase.value == SolveInfo::noINFO || phase.value == SolveInfo::pFEAS || phase.value == SolveInfo::dFEAS) {
...
AverageComplementarity::AverageComplementarity(dd_real lambdaStar) { initialize(lambdaStar); }

AverageComplementarity::~AverageComplementarity() {
    // Nothing needs.
}

void AverageComplementarity::initialize(dd_real lambdaStar) {
...
RatioInitResCurrentRes::RatioInitResCurrentRes(Parameter &param, Residuals &initRes) { initialize(param, initRes); }

void RatioInitResCurrentRes::initialize(Parameter &param, Residuals &initRes) {
    dd_real accuracy = param.epsilonDash;
...
void RatioInitResCurrentRes::update(Switch &reduction, StepLength &alpha) {
...
void RatioInitResCurrentRes::update_exact(Residuals &initRes, Residuals &currentRes) {
...
SolveInfo::SolveInfo(InputData &inputData, Solutions &currentPt, dd_real mu0, dd_real omegaStar) { initialize(inputData, currentPt, mu0, omegaStar); }

SolveInfo::~SolveInfo() {
    // Nothing needs.
}

void SolveInfo::initialize(InputData &inputData, Solutions &currentPt, dd_real mu0, dd_real omegaStar) {
    int nDim = currentPt.nDim;
    Vector &b = inputData.b;
    SparseLinearSpace &C = inputData.C;
...
void SolveInfo::update(InputData &inputData, DenseLinearSpace &initPt_xMat, DenseLinearSpace &initPt_zMat, Solutions &currentPt, Residuals &currentRes, AverageComplementarity &mu, RatioInitResCurrentRes &theta, Parameter &param) {
    int nDim = currentPt.nDim;
    Vector &b = inputData.b;
    SparseLinearSpace &C = inputData.C;
    ...
    Lal::let(objValPrimal, '=', C, '.', currentPt.xMat);
    Lal::let(objValDual, '=', b, '.', currentPt.yVec);
    dd_real primal = theta.primal;
    dd_real dual = theta.dual;
    dd_real omega = param.omegaStar;
    rho = 0.0;
    dd_real x0z0 = nDim * mu.initial;
    dd_real xMatzMat = nDim * mu.current;
    dd_real x0zMat = 0.0;
    dd_real xMatz0 = 0.0;
    ...
    if (currentRes.normPrimalVec > accuracy && currentRes.normDualMat <= accuracy) {
        rho = primal * x0zMat / ((primal + (1.0 - primal) * omega) * etaDual + xMatzMat);
    }

    // primal is feasible and dual is infeasible
    if (currentRes.normPrimalVec <= accuracy && currentRes.normDualMat > accuracy) {
        rho = dual * xMatz0 / ((dual + (1.0 - dual) * omega) * etaPrimal + xMatzMat);
    }

    // primal and dual are infeasible
    if (currentRes.normPrimalVec > accuracy && currentRes.normDualMat > accuracy) {
        rho = (dual * xMatz0 + primal * x0zMat) / ((primal * dual + omega * (primal * (1.0 - dual) + (1.0 - primal) * dual)) * x0z0 + xMatzMat);
    }
    // rMessage("eta Primal = " << etaPrimal);
    // rMessage("eta Dual = " << etaDual);
}

void SolveInfo::update(double &lambda, InputData &inputData, Solutions &currentPt, Residuals &currentRes, AverageComplementarity &mu, RatioInitResCurrentRes &theta, Parameter &param) {
    int nDim = currentPt.nDim;
    Vector &b = inputData.b;
    SparseLinearSpace &C = inputData.C;
...
//     gap                         gap,     mu * nDim
void SolveInfo::check(InputData &inputData, Solutions &currentPt, Residuals &currentRes, AverageComplementarity &mu, RatioInitResCurrentRes &theta, Parameter &param) {
    dd_real tmp, tmp1p, tmp1d, tmp2p, tmp2d, tmp3p, tmp3d, tmp4, tmp5p, tmp5d;
...
Phase::Phase(Residuals &initRes, SolveInfo &solveInfo, Parameter &param, int nDim) { initialize(initRes, solveInfo, param, nDim); }

Phase::~Phase() {
    // Nothing needs.
}

bool Phase::initialize(Residuals &initRes, SolveInfo &solveInfo, Parameter &param, int nDim) {
    this->nDim = nDim;
    return updateCheck(initRes, solveInfo, param);
}

bool Phase::updateCheck(Residuals &currentRes, SolveInfo &solveInfo, Parameter &param) {
    const dd_real NONZERO = 1.0e-6;
    dd_real accuracy = param.epsilonDash;
    value = SolveInfo::noINFO;
    ...
    if (value == SolveInfo::noINFO && currentRes.normDualMat <= accuracy) {
    ...
    if (value == SolveInfo::noINFO && solveInfo.rho > 1.0 + NONZERO) {
...
#if REVERSE_PRIMAL_DUAL
    switch (value) {
    case SolveInfo::noINFO:;
        break;
    case SolveInfo::pFEAS:
        value = SolveInfo::dFEAS;
        break;
    case SolveInfo::dFEAS:
        value = SolveInfo::pFEAS;
        break;
    case SolveInfo::pdFEAS:;
        break;
    case SolveInfo::pdINF:;
        break;
    case SolveInfo::pFEAS_dINF:
        value = SolveInfo::pINF_dFEAS;
        break;
    case SolveInfo::pINF_dFEAS:
        value = SolveInfo::pFEAS_dINF;
        break;
    case SolveInfo::pdOPT:;
        break;
    case SolveInfo::pUNBD:
        value = SolveInfo::dUNBD;
        break;
    case SolveInfo::dUNBD:
        value = SolveInfo::pUNBD;
        break;
    default:
        break;
